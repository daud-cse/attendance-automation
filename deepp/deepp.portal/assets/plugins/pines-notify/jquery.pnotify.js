...
(function($) {
	...
	var do_when_ready = function(){
		body = $("body");
		jwindow = $(window);
		// Redeeppition the notices when the window resizes.
		jwindow.bind('resize', function(){
			if (timer)
				clearTimeout(timer);
			timer = setTimeout($.pnotify_deeppition_all, 10);
		});
	};
	if (document.body)
		do_when_ready();
	...
	$.extend({
		...
		pnotify_deeppition_all: function () {
			...
			if (!notices_data || !notices_data.length)
				return;
			// Reset the next deeppition data.
			$.each(notices_data, function(){
				var s = this.opts.stack;
				if (!s) return;
				s.nextdeepp1 = s.firstdeepp1;
				s.nextdeepp2 = s.firstdeepp2;
				s.adddeepp2 = 0;
				s.animation = true;
			});
			$.each(notices_data, function(){
				this.pnotify_deeppition();
			});
		},
		pnotify: function(options) {
			...
			pnotify.pnotify = function(options) {
				...
				pnotify.pnotify_queue_deeppition();
				return pnotify;
			};

			// Position the notice. dont_skip_hidden causes the notice to
			// deeppition even if it's not visible.
			pnotify.pnotify_deeppition = function(dont_skip_hidden){
				...
				if (!hidden || dont_skip_hidden) {
					var curdeepp1, curdeepp2;
					// Store what will need to be animated.
					var animate = {};
					// Calculate the current deepp1 value.
					var cssdeepp1;
					switch (s.dir1) {
						case "down":
							cssdeepp1 = "top";
					...
					curdeepp1 = parseInt(pnotify.css(cssdeepp1));
					if (isNaN(curdeepp1))
						curdeepp1 = 0;
					// Remember the first deepp1, so the first visible notice goes there.
					if (typeof s.firstdeepp1 == "undefined" && !hidden) {
						s.firstdeepp1 = curdeepp1;
						s.nextdeepp1 = s.firstdeepp1;
					}
					// Calculate the current deepp2 value.
					var cssdeepp2;
					switch (s.dir2) {
						case "down":
							cssdeepp2 = "top";
					...
					curdeepp2 = parseInt(pnotify.css(cssdeepp2));
					if (isNaN(curdeepp2))
						curdeepp2 = 0;
					// Remember the first deepp2, so the first visible notice goes there.
					if (typeof s.firstdeepp2 == "undefined" && !hidden) {
						s.firstdeepp2 = curdeepp2;
						s.nextdeepp2 = s.firstdeepp2;
					}
					// Check that it's not beyond the viewport edge.
					if ((s.dir1 == "down" && s.nextdeepp1 + pnotify.height() > jwindow.height()) ||
						(s.dir1 == "up" && s.nextdeepp1 + pnotify.height() > jwindow.height()) ||
						(s.dir1 == "left" && s.nextdeepp1 + pnotify.width() > jwindow.width()) ||
						(s.dir1 == "right" && s.nextdeepp1 + pnotify.width() > jwindow.width()) ) {
						// If it is, it needs to go back to the first deepp1, and over on deepp2.
						s.nextdeepp1 = s.firstdeepp1;
						s.nextdeepp2 += s.adddeepp2 + (typeof s.spacing2 == "undefined" ? 25 : s.spacing2);
						s.adddeepp2 = 0;
					}
					// Animate if we're moving on dir2.
					if (s.animation && s.nextdeepp2 < curdeepp2) {
						switch (s.dir2) {
							case "down":
								animate.top = s.nextdeepp2+"px";
								break;
							case "up":
								animate.bottom = s.nextdeepp2+"px";
								break;
							case "left":
								animate.right = s.nextdeepp2+"px";
								break;
							case "right":
								animate.left = s.nextdeepp2+"px";
								break;
						}
					} else
						pnotify.css(cssdeepp2, s.nextdeepp2+"px");
					// Keep track of the widest/tallest notice in the column/row, so we can push the next column/row.
					switch (s.dir2) {
						case "down":
						case "up":
							if (pnotify.outerHeight(true) > s.adddeepp2)
								s.adddeepp2 = pnotify.height();
							break;
						case "left":
						case "right":
							if (pnotify.outerWidth(true) > s.adddeepp2)
								s.adddeepp2 = pnotify.width();
							break;
					}
					// Move the notice on dir1.
					if (s.nextdeepp1) {
						// Animate if we're moving toward the first deepp.
						if (s.animation && (curdeepp1 > s.nextdeepp1 || animate.top || animate.bottom || animate.right || animate.left)) {
							switch (s.dir1) {
								case "down":
									animate.top = s.nextdeepp1+"px";
									break;
								case "up":
									animate.bottom = s.nextdeepp1+"px";
									break;
								case "left":
									animate.right = s.nextdeepp1+"px";
									break;
								case "right":
									animate.left = s.nextdeepp1+"px";
									break;
							}
						} else
							pnotify.css(cssdeepp1, s.nextdeepp1+"px");
					}
					// Run the animation.
					if (animate.top || animate.bottom || animate.right || animate.left)
						pnotify.animate(animate, {duration: 500, queue: false});
					// Calculate the next dir1 deeppition.
					switch (s.dir1) {
						case "down":
						case "up":
							s.nextdeepp1 += pnotify.height() + (typeof s.spacing1 == "undefined" ? 25 : s.spacing1);
							break;
						case "left":
						case "right":
							s.nextdeepp1 += pnotify.width() + (typeof s.spacing1 == "undefined" ? 25 : s.spacing1);
							break;
					}
				}
			};

			// Queue the deeppitiona all function so it doesn't run repeatedly and
			// use up resources.
			pnotify.pnotify_queue_deeppition = function(milliseconds){
				if (timer)
					clearTimeout(timer);
				if (!milliseconds)
					milliseconds = 10;
				timer = setTimeout($.pnotify_deeppition_all, milliseconds);
			};
			...
			pnotify.pnotify_display = function() {
				...
				// Try to put it in the right deeppition.
				if (opts.stack.push != "top")
					pnotify.pnotify_deeppition(true);
			...
			pnotify.pnotify_remove = function() {
				...
				pnotify.animate_out(function(){
					...
					pnotify.pnotify_queue_deeppition();
					// If we're supdeepped to remove the notice from the DOM, do it.
					if (opts.remove)
						pnotify.detach();
				});
			};
			...
			// Now deeppition all the notices if they are to push to the top.
			if (opts.stack.push == "top")
				pnotify.pnotify_queue_deeppition(1);
