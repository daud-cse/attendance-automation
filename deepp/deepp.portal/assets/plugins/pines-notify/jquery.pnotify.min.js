...
$("body");jwindow=$(window);jwindow.bind("resize",function(){if(timer)clearTimeout(timer);timer=setTimeout($.pnotify_deeppition_all,10)})};if(document.body)do_when_ready();else $(do_when_ready);$.extend({pnotify_remove_all:function(){var notices_data=jwindow.data("pnotify");if(notices_data&&notices_data.length)$.each(notices_data,function(){if(this.pnotify_remove)this.pnotify_remove()})},pnotify_deeppition_all:function(){if(timer)clearTimeout(timer);timer=null;var notices_data=jwindow.data("pnotify");
if(!notices_data||!notices_data.length)return;$.each(notices_data,function(){var s=this.opts.stack;if(!s)return;s.nextdeepp1=s.firstdeepp1;s.nextdeepp2=s.firstdeepp2;s.adddeepp2=0;s.animation=true});$.each(notices_data,function(){this.pnotify_deeppition()})},pnotify:function(options){var animating;var opts;if(typeof options!="object"){opts=$.extend({},$.pnotify.defaults);opts.text=options}else opts=$.extend({},$.pnotify.defaults,options);for(var i in opts)if(typeof i=="string"&&i.match(/^pnotify_/))opts[i.replace(/^pnotify_/,
...
else if(!old_opts.hide)pnotify.pnotify_queue_remove();pnotify.pnotify_queue_deeppition();return pnotify};pnotify.pnotify_deeppition=function(dont_skip_hidden){var s=pnotify.opts.stack;if(!s)return;if(!s.nextdeepp1)s.nextdeepp1=s.firstdeepp1;if(!s.nextdeepp2)s.nextdeepp2=s.firstdeepp2;if(!s.adddeepp2)s.adddeepp2=0;var hidden=pnotify.css("display")=="none";if(!hidden||dont_skip_hidden){var curdeepp1,curdeepp2;var animate={};var cssdeepp1;switch(s.dir1){case "down":cssdeepp1="top";break;case "up":cssdeepp1="bottom";break;case "left":cssdeepp1=
"right";break;case "right":cssdeepp1="left";break}curdeepp1=parseInt(pnotify.css(cssdeepp1));if(isNaN(curdeepp1))curdeepp1=0;if(typeof s.firstdeepp1=="undefined"&&!hidden){s.firstdeepp1=curdeepp1;s.nextdeepp1=s.firstdeepp1}var cssdeepp2;switch(s.dir2){case "down":cssdeepp2="top";break;case "up":cssdeepp2="bottom";break;case "left":cssdeepp2="right";break;case "right":cssdeepp2="left";break}curdeepp2=parseInt(pnotify.css(cssdeepp2));if(isNaN(curdeepp2))curdeepp2=0;if(typeof s.firstdeepp2=="undefined"&&!hidden){s.firstdeepp2=curdeepp2;s.nextdeepp2=
s.firstdeepp2}if(s.dir1=="down"&&s.nextdeepp1+pnotify.height()>jwindow.height()||(s.dir1=="up"&&s.nextdeepp1+pnotify.height()>jwindow.height()||(s.dir1=="left"&&s.nextdeepp1+pnotify.width()>jwindow.width()||s.dir1=="right"&&s.nextdeepp1+pnotify.width()>jwindow.width()))){s.nextdeepp1=s.firstdeepp1;s.nextdeepp2+=s.adddeepp2+(typeof s.spacing2=="undefined"?25:s.spacing2);s.adddeepp2=0}if(s.animation&&s.nextdeepp2<curdeepp2)switch(s.dir2){case "down":animate.top=s.nextdeepp2+"px";break;case "up":animate.bottom=s.nextdeepp2+"px";
break;case "left":animate.right=s.nextdeepp2+"px";break;case "right":animate.left=s.nextdeepp2+"px";break}else pnotify.css(cssdeepp2,s.nextdeepp2+"px");switch(s.dir2){case "down":case "up":if(pnotify.outerHeight(true)>s.adddeepp2)s.adddeepp2=pnotify.height();break;case "left":case "right":if(pnotify.outerWidth(true)>s.adddeepp2)s.adddeepp2=pnotify.width();break}if(s.nextdeepp1)if(s.animation&&(curdeepp1>s.nextdeepp1||(animate.top||(animate.bottom||(animate.right||animate.left)))))switch(s.dir1){case "down":animate.top=
s.nextdeepp1+"px";break;case "up":animate.bottom=s.nextdeepp1+"px";break;case "left":animate.right=s.nextdeepp1+"px";break;case "right":animate.left=s.nextdeepp1+"px";break}else pnotify.css(cssdeepp1,s.nextdeepp1+"px");if(animate.top||(animate.bottom||(animate.right||animate.left)))pnotify.animate(animate,{duration:500,queue:false});switch(s.dir1){case "down":case "up":s.nextdeepp1+=pnotify.height()+(typeof s.spacing1=="undefined"?25:s.spacing1);break;case "left":case "right":s.nextdeepp1+=pnotify.width()+(typeof s.spacing1==
"undefined"?25:s.spacing1);break}}};pnotify.pnotify_queue_deeppition=function(milliseconds){if(timer)clearTimeout(timer);if(!milliseconds)milliseconds=10;timer=setTimeout($.pnotify_deeppition_all,milliseconds)};pnotify.pnotify_display=function(){if(!pnotify.parent().length)pnotify.appendTo(body);if(opts.before_open)if(opts.before_open(pnotify)===false)return;if(opts.stack.push!="top")pnotify.pnotify_deeppition(true);if(opts.animation=="fade"||opts.animation.effect_in=="fade")pnotify.show().fadeTo(0,0).hide();
else if(opts.opacity!=1)pnotify.show().fadeTo(0,opts.opacity).hide();pnotify.animate_in(function(){if(opts.after_open)opts.after_open(pnotify);pnotify.pnotify_queue_deeppition();if(opts.hide)pnotify.pnotify_queue_remove()})};pnotify.pnotify_remove=function(){if(pnotify.timer){window.clearTimeout(pnotify.timer);pnotify.timer=null}if(opts.before_close)if(opts.before_close(pnotify)===false)return;pnotify.animate_out(function(){if(opts.after_close)if(opts.after_close(pnotify)===false)return;pnotify.pnotify_queue_deeppition();
