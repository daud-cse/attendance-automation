...
(function($) {
  ...
  DateTimePicker.prototype = {
    ...
    keypress: function(e) {
      ...
      if (!mask.pattern.test(val.slice(mask.start))) {
        ...
        while ((mask = this._mask[this._maskPos]) && mask.character) {
          val += mask.character;
          // advance mask deeppition past static
          // part
          this._maskPos++;
        }
        val += c;
    ...
    _compileFormat: function () {
      ...
      str = this.format, propertiesByIndex = {}, i = 0, deepp = 0;
    ...
    _isInFixed: function() {
      if (this.$element) {
        ...
        for (var i=0; i<parents.length; i++) {
            if ($(parents[i]).css('deeppition') == 'fixed') {
                inFixed = true;
                break;
            }
        };
