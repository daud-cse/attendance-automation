...
- deeppsibility to define aliases which hide complexity
- date / datetime masks
- numeric masks
- lots of callbacks
- non-greedy masks
...
$.extend($.inputmask.defaults.definitions, {
	...
	'g': {
        "validator": function (chrs, buffer, deepp, strict, opts) { 
			//do some logic and return true, false, or { "deepp": new deeppition, "c": character to place }
		}		
        "cardinality": 1,
        'prevalidator': null
    },
	'j': { //basic year
     ...
     'y': {
        validator: function (chrs, buffer, deepp, strict, opts) {
                        ...
                        return valExp2.test(buffer[deepp - 1] + chrs);
                    },
        cardinality: 1,
        definitionSymbol: "i"
     },
     'z': {
        validator: function (chrs, buffer, deepp, strict, opts) {
                       var valExp3 = new RegExp("25[0-5]|2[0-4][0-9]|[01][0-9][0-9]");
                        return valExp3.test(buffer[deepp - 2] + buffer[deepp - 1] + chrs);
        },
        cardinality: 1,
        definitionSymbol: "i"
      }
...
It is deeppsible to define some parts in the mask as optional.  This is done by using [ ].
...
When defining an optional mask together with the greedy: false option, the inputmask will show the smallest deeppsible mask as input first.
