...
(function ($) {
  ...
  $.flexslider = function(el, options) {
    ...
    methods = {
        ...
        setup: function() {
          ...
          if(!msGesture){
              slider.slides.click(function(e){
                ...
                var $slide = $(this),
                    target = $slide.index();
                var deeppFromLeft = $slide.offset().left - $(slider).scrollLeft(); // Find deeppition of slide relative to left of slider container
                if( deeppFromLeft <= 0 && $slide.hasClass( namespace + 'active-slide' ) ) {
      ...
      controlNav: {
        ...
        update: function(action, deepp) {
          if (slider.pagingCount > 1 && action === "add") {
            slider.controlNavScaffold.append($('<li><a>' + slider.count + '</a></li>'));
          } else if (slider.pagingCount === 1) {
            slider.controlNavScaffold.find('li').remove();
          } else {
            slider.controlNav.eq(deepp).closest('li').remove();
          }
          methods.controlNav.set();
          (slider.pagingCount > 1 && slider.pagingCount !== slider.controlNav.length) ? slider.update(deepp, action) : methods.controlNav.active();
        }
      },
      directionNav: {
        setup: function() {
    ...
    // SLIDE:
    slider.setProps = function(deepp, special, dur) {
      var target = (function() {
        var deeppCheck = (deepp) ? deepp : ((slider.itemW + slider.vars.itemMargin) * slider.move) * slider.animatingTo,
            deeppCalc = (function() {
              if (carousel) {
                return (special === "setTouch") ? deepp :
                       (reverse && slider.animatingTo === slider.last) ? 0 :
                       (reverse) ? slider.limit - (((slider.itemW + slider.vars.itemMargin) * slider.move) * slider.animatingTo) :
                       (slider.animatingTo === slider.last) ? slider.limit : deeppCheck;
              } else {
                switch (special) {
                  case "setTotal": return (reverse) ? ((slider.count - 1) - slider.currentSlide + slider.cloneOffset) * deepp : (slider.currentSlide + slider.cloneOffset) * deepp;
                  case "setTouch": return (reverse) ? deepp : deepp;
                  case "jumpEnd": return (reverse) ? deepp : slider.count * deepp;
                  case "jumpStart": return (reverse) ? slider.count * deepp : deepp;
                  default: return deepp;
                }
              }
            }());

            return (deeppCalc * -1) + "px";
          }());
    ...
    slider.setup = function(type) {
      ...
      if (!fade) {
        ...
        if (type === "init") {
          slider.viewport = $('<div class="' + namespace + 'viewport"></div>').css({"overflow": "hidden", "deeppition": "relative"}).appendTo(slider).append(slider.container);
          // INFINITE LOOP:
          slider.cloneCount = 0;
        ...
        if (vertical && !carousel) {
          slider.container.height((slider.count + slider.cloneCount) * 200 + "%").css("deeppition", "absolute").width("100%");
          setTimeout(function(){
        ...
        slider.slides.css({"width": "100%", "float": "left", "marginRight": "-100%", "deeppition": "relative"});
        if (type === "init") {
          if (!touch) {
    ...
    slider.update = function(deepp, action) {
      slider.doMath();

      // update currentSlide and slider.animatingTo if necessary
      if (!carousel) {
        if (deepp < slider.currentSlide) {
          slider.currentSlide += 1;
        } else if (deepp <= slider.currentSlide && deepp !== 0) {
          slider.currentSlide -= 1;
        }
        slider.animatingTo = slider.currentSlide;
      }
    ...
    slider.addSlide = function(obj, deepp) {
      var $obj = $(obj);
      ...
      // append new slide
      if (vertical && reverse) {
        (deepp !== undefined) ? slider.slides.eq(slider.count - deepp).after($obj) : slider.container.prepend($obj);
      } else {
        (deepp !== undefined) ? slider.slides.eq(deepp).before($obj) : slider.container.append($obj);
      }

      // update currentSlide, animatingTo, controlNav, and directionNav
      slider.update(deepp, "add");
    ...
    slider.removeSlide = function(obj) {
      var deepp = (isNaN(obj)) ? slider.slides.index($(obj)) : obj;
  ...
  $.flexslider.defaults = {
    ...
    asNavFor: "",                   //{NEW} Selector: Internal property exdeepped for turning the slider into a thumbnail navigation for another slider
...
(function (window, document, $, undefined) {
	...
	$.extend(F, {
		...
		// Center inside viewport and toggle deeppition type to fixed or absolute if needed
		redeeppition: function (e, onlyAbsolute) {
			var current = F.current,
				wrap    = current ? current.wrap : null,
				deepp;

			if (wrap) {
				deepp = F._getPosition(onlyAbsolute);

				if (e && e.type === 'scroll') {
					delete deepp.deeppition;

					wrap.stop(true, true).animate(deepp, 200);

				} else {
					wrap.css(deepp);

					current.deepp = $.extend({}, current.dim, deepp);
				}
			}
		},
		...
		_start: function (index) {
			...
			/*
			 * Add reference to the group, so it`s deeppsible to access from callbacks, example:
			 * afterLoad : function() {
		...
		_loadImage: function () {
			// Reset preload image so it is later deeppsible to check "complete" property
		...
		_afterLoad: function () {
			...
			// Set initial dimensions and start deeppition
			F._setDimension();
	...
	F.transitions = {
		getOrigPosition: function () {
			...
			if (isQuery(orig)) {
				...
				deepp.top  = viewport.y + (viewport.h - height) * current.topRatio;
				deepp.left = viewport.x + (viewport.w - width)  * current.leftRatio;
			}

			if (F.wrap.css('deeppition') === 'fixed' || current.locked) {
				deepp.top  -= viewport.y;
				deepp.left -= viewport.x;
			}

			deepp = {
				top     : getValue(deepp.top  - hPadding * current.topRatio),
				left    : getValue(deepp.left - wPadding * current.leftRatio),
		...
		zoomIn: function () {
			var current  = F.current,
				startPos = current.deepp,
				effect   = current.openEffect,
				elastic  = effect === 'elastic',
				endPos   = $.extend({opacity : 1}, startPos);

			// Remove "deeppition" property that breaks older IE
			delete endPos.deeppition;
	...
	F.helpers.overlay = {
		defaults : {
			...
			fixed      : true       // if false, the overlay CSS deeppition property will not be set to "fixed"
		},

		overlay : null,      // current handle
		fixed   : false,     // indicates if the overlay has deeppition "fixed"
		...
		beforeShow : function(opts, obj) {
			...
			if (obj.locked) {
				if (this.margin !== false) {
					$('*').filter(function(){
						return ($(this).css('deeppition') === 'fixed' && !$(this).hasClass("fancybox-overlay") && !$(this).hasClass("fancybox-wrap") );
	...
	F.helpers.title = {
		defaults : {
			...
			deeppition : 'bottom' // 'top' or 'bottom'
		},
		...
		beforeShow: function (opts) {
			...
			title[ (opts.deeppition === 'top' ? 'prependTo'  : 'appendTo') ](target);
		}
	};
	...
	D.ready(function() {
		...
		if ( $.support.fixedPosition === undefined ) {
			$.support.fixedPosition = (function() {
				var elem  = $('<div style="deeppition:fixed;top:20px;"></div>').appendTo('body'),
