...
namespace pnsms.Service
{
    ...
    public class GalleryService : Service<Gallery>, IGalleryService
    {
        ...
        public GalleryService(IRepositoryAsync<Gallery> repository, IImageService iImageService, IEventService eventService)
            : base(repository)
        {
        ...
        public IEnumerable<Gallery> GetAllGallery(int instituteId)
        {
            ...
            var gallery = _repository.Query(x => x.InstituteId == instituteId).Select();
            return gallery;
        }
        /// <summary>
        ...
        public IEnumerable<Gallery> GetGalleryByInstituteId(int instituteId)
        {
            ...
            var gallery = _repository.Query(x => x.InstituteId == instituteId).Select();//.Where(x => x.InstituteId == InstituteId);
            return gallery;
        }
        /// <summary>
        ...
        public Gallery GetGalleryById(int instituteId, int id)
        {
            ...
            var gallery = _repository.Query(x => x.InstituteId == instituteId && x.Id == id).Select().FirstOrDefault();
            if (gallery == null)
            {
        ...
        public Gallery GetGalleryDefault(int instituteId, bool IsActive)
        {
           ...
           // List<Gallery> galleryList = new List<Gallery>();
            ...
            var galleryList = _repository.Query(x => x.InstituteId == instituteId && x.IsActive == IsActive).Select().ToList().Take(9).OrderByDescending(x=>x.LastUpdateTime);          
        ...
        public List<Gallery> GetGlobalGalleryByCurrentDateWithenStartEndDate(int instituteId, bool isActive)
        {
            ...
            // var notice
            var gallery = _repository.Query().Select()
        ...
        public List<Gallery> GetActiveGallery(int instituteId)
        {
            ...
            Image image = new Image();
            var gallery = _repository.Query(x => x.InstituteId == instituteId && x.IsActive==true).Select().ToList();
            foreach (Gallery item in gallery)
            {
                image = _iImageService.GetImageByRefTypeIdAndRefPrimaryKey(((int)RefCode.Gallery), item.Id).ToList().FirstOrDefault();
                item.Image = image;
        ...
        {
            // List<Gallery> GalleryList = new List<Gallery>();
            var gallery = _repository.Query(x => x.InstituteId == instituteId && x.IsActive == true).Select();
