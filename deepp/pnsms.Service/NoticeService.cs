...
namespace pnsms.Service
{
  public interface INoticeService
   ...
   {
       private readonly IRepositoryAsync<Notice> _repository;
       private readonly IImageService _imageService;
       private readonly INoticeTypeService _noticeTypeService;
       List<Notice> Notice = new List<Notice>();


       public NoticeService(IRepositoryAsync<Notice> repository, IImageService imageService, INoticeTypeService noticeTypeService)
           : base(repository)
       {
       ...
       public IEnumerable<Notice> GetAllNotice(int instituteId)
        {
            var notice = _repository.Query(x => x.InstituteId == instituteId).Select();
           return notice;
       }

       public IEnumerable<Notice> GetNoticeByEmployeeId(int instituteId, int EmployeeId)
        {
           var notice = _repository.Query().Select();
           return notice;
       }
       public IEnumerable<Notice> GetNoticeByInstituteId(int InstituteId)
        {
           var notice = _repository.Query().Select().Where(x => x.InstituteId == InstituteId);
           return notice;
       }
       ...
       public Notice GetNoticeById(int id)
        {
            var notice = _repository.Query(x => x.Id == id).Select().FirstOrDefault();
            return notice;
        }
       ...
       public IEnumerable<Notice> GetAllNoticeListBySearch(VmSearch<Notice> noticeModel)
       {
           ...
           var noticeList = _repository.Query(c => c.InstituteId == noticeModel.InstituteId 
               &&
       ...
       public void SaveNotice(IUnitOfWorkAsync unitOfWorkAsync, List<byte[]> imageList, Notice noticeModel)
       {
               ...
               _repository.Insert(noticeModel);
       ...
       {
            var notic = _repository.Query(x => x.InstituteId == instituteId && x.IsActive == true).Select();
            return notic;
        }
       
       public IEnumerable<Notice> GetActiveNoticeByTypeId(int instituteId, int typeId)
        {
            var notic = _repository.Query(x => x.InstituteId == instituteId && x.IsActive == true && x.NoticeTypeId == typeId).Select();
           return notic;
       }
