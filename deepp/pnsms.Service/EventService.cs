...
namespace pnsms.Service
{
    ...
    public class EventService : Service<Event>, IEventService
    {
        ...
        public IEnumerable<Event> GetAllEvent()
        {
            ...
            var Event = _repository.Query().Select();
            return Event;
        }
        public IEnumerable<Event> GetAllEvent(int instituteId)
        {
            var Event = _repository.Query(x => x.InstituteId == instituteId).Select();
            return Event;
        }
        ...
        {
            var Event = _repository.Query().Select().Where(x => x.InstituteId == InstituteId);
            return Event;
        }
        ...
        public Event GetEventById(int id)
        {
            ...
            var Event = _repository.Query().Include(x=>x.Institute).Select().Where(x => x.Id == id).SingleOrDefault();
          //  Event.Images=Images;
        ...
        public IEnumerable<Event> GetAllEventListBySearch(VmSearch<Event> eventModel)
        {
            ...
            var eventList = _repository.Query()
                .Select()
        ...
        {
            var Event = _repository.Query(x => x.InstituteId == instituteId && x.IsActive==true).Select();
            return Event;
        }
