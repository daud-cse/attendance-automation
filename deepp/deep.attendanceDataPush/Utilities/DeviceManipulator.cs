...
namespace deepp.attendance
{
    internal class DeviceManipulator
    {
        ...
        //public string GetAttendanceRule(ZkemClient objZkeeper,int machineNumber)
        //{
        //    StringBuilder sb = new StringBuilder();

        //    sb= objZkeeper.ReadAttRule(machineNumber);
        //}

        public ICollection<MachineInfo> GetAllUserInfo(ZkemClient objZkeeper, int machineNumber, string deviceinfo)
        {
            string Name = "";
            string Password = "";
            int Privilege = 0;
            ...
            ICollection<MachineInfo> lstUserInfo = new List<MachineInfo>();

            //objZkeeper.ReadAllGLogData(machineNumber);
            objZkeeper.ReadAllUserID(machineNumber);
            while (objZkeeper.SSR_GetUserInfo(machineNumber, dwEnrollNumber, out Name, out Password, out Privilege, out Enabled))


            {
                //string inputDate = dwYear + "-" + dwMonth + "-" + dwDay + "-" + dwHour + "-" + dwMinute + "-" + dwSecond;//new DateTime(dwYear, dwMonth, dwDay, dwHour, dwMinute, dwSecond).ToString();
             ...
             //   objInfo.DateTimeRecord = inputDate;
                objInfo.StudentOrTeacherId = Name;
                lstUserInfo.Add(objInfo);
            }
        ...
        public ICollection<MachineInfo> GetLogData(ZkemClient objZkeeper, int machineNumber,string deviceinfo)
        {
