...
namespace deepp.Service.Settings
{
    ...
    {
        private readonly IRepositoryAsync<AcademicClassSectionMapping> redeeppitory;
        ...
        public AcademicClassSectionMappingService(IRepositoryAsync<AcademicClassSectionMapping> redeeppitory, IAcademicBranchService academicBranchService,
        ...
        {
            return redeeppitory.Query(c => c.InstituteId == instituteId
        ...
        {
            return redeeppitory.Query(c => c.Id == id)
                .Select().SingleOrDefault();
        }
        ...
        {
            var result = redeeppitory.Query(c => c.InstituteId == instituteId
        ...
        {
            var result = redeeppitory.Query(c => c.InstituteId == instituteId && (c.AcademicBranchId == branchId)
        ...
        public List<KeyValuePair<int,string>> Getkvp(int instituteId, int classId)
        {
            ...
            var kvpAcademicClassSectionMapping = new List<KeyValuePair<int, string>>();
             redeeppitory.Query(c => c.InstituteId == instituteId
                && (c.AcademicClassId == classId))
        ...
        {
            var result = redeeppitory.Query(c => c.InstituteId == instituteId) .Select();
            ...
            try
            {
                ...
                foreach (var objAcademicClassSectionMapping in academicClassSectionMapping)
                {
                    ...
                    if (objAcademicClassSectionMapping1.Id > 0)
                    {
                        ...
                        {
                            redeeppitory.Update(objAcademicClassSectionMapping1);

                        }
                        else
                        {
                            redeeppitory.Delete(objAcademicClassSectionMapping1);
                        }
                    }
                    else
                    {
                        if (objAcademicClassSectionMapping1.IsActive)
                        {
                            redeeppitory.Insert(objAcademicClassSectionMapping1);
        ...
        public void Update(IUnitOfWorkAsync unitOfWorkAsync, AcademicClassSectionMapping RoutinePeriodByClass)
        {
            try
            {
                ...
                redeeppitory.Update(RoutinePeriodByClass);
                unitOfWorkAsync.SaveChanges();
