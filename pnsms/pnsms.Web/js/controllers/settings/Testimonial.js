...
app.controller('TestimonialCtrl', ['$scope', '$http', 'TestimonialService', '$filter', 'toaster', function ($scope, $http, TestimonialService, $filter, toaster) {

    // init
    $scope.entity = { Name: "Testimonial" };
    ...
    $scope.search = function () {
        ...
        $scope.filteredItems = $filter('filter')($scope.items, function (item) {
            ...
            if (searchMatch(item["TestimonialBy"], $scope.query))
                return true;
            ///}
            return false;
        });
    ...
    $scope.GetAll = function () {
        TestimonialService.query(function (Testimoniales) {
            $scope.Testimoniales = Testimoniales;
            $scope.items = Testimoniales;
            $scope.query = "";
            $scope.search();
    ...
    // inline Add /Edit
    $scope.GetAll();
    $scope.Testimonial = GetNewModel();
    $scope.Add = function (item) {
        console.log(item);
        TestimonialService.save(item, function (result) {
    ...
    $scope.Update = function (item) {
        TestimonialService.update({ id: item.Id }, item, function (result) {
...
function GetNewModel() {
    return { Id: 0, IsActive: true, TestimonialBy: "", TestimonialBody: "" };
