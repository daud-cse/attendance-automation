...
/* Controllers */
// Exam controller
app.controller('RelatedAccountCtrl', ['$scope', '$http', 'RelatedAccountService', '$filter', 'toaster', function ($scope, $http, RelatedAccountService, $filter, toaster) {

    // init
    $scope.entity = { Name: "Related Account" };
    ...
    $scope.isNew = false;
    $scope.selected = {};
    $scope.newRelatedAccount = null;
    $scope.RelatedAccount = {};
    ...
    $scope.GetAll = function () {
        // alert("sds");
        RelatedAccountService.query(function (RelatedAccount) {



            $scope.RelatedAccount = RelatedAccount // Exam.query();

            $scope.items = RelatedAccount;
            $scope.query = "";
            $scope.search();
            $scope.isEdit = false;
    ...
    $scope.getRelatedAccountTypeId = function () {

        if ($scope.RelatedAccount.RelatedAccountTypeId != null) {
            RelatedAccountService.getRelatedAccountTypeId({ id: $scope.RelatedAccount.RelatedAccountTypeId }, function (result) {

                $scope.RelatedAccountTypeIdList = result;
                $scope.newRelatedAccount.RelatedAccountTypeIdList = $scope.RelatedAccountTypeIdList;
    ...
    $scope.RelatedAccountTypeIdList = [];
    $scope.New = function () {

        RelatedAccountService.New(function (RelatedAccount) {
            $scope.newRelatedAccount = RelatedAccount;
           // $scope.newBankAccount.kvpBankAccountType = BankAccount.kvpBankAccountType;
            ...
            // $scope.BankAccountTypeIdList = BankAccount.kvpBankAccountType;
            ////console.log(list[0].Key);        
            ////console.log(list[0].Value);

            ////$scope.NewBankAccount = BankAccount;

            //console.log(BankAccountTypeIdList);
            ////console.log(list[0].Value);
    ...
    $scope.Save = function (RelatedAccount) {
        //  alert("safds");
        RelatedAccountService.save(RelatedAccount, function (result) {
    ...
    $scope.Edit = function (item) {
        ...
        RelatedAccountService.New({ id: item.AccountId }, function (RelatedAccount) {
            $scope.newRelatedAccount = RelatedAccount;
            //$scope.newBankAccount.kvpBankAccountType = BankAccount.kvpBankAccountType;
        });
    };
    $scope.Update = function (item) {
        ...
        RelatedAccountService.update({ id: item.ID }, item, function (result) {
