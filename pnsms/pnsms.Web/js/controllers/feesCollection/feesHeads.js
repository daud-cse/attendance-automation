...
'use strict';
/* Controllers */
// feesHeads controller
app.controller('feesHeadsCtrl', ['$scope', '$http', 'feesHeadsService', '$filter', 'toaster', function ($scope, $http, feesHeadsService, $filter, toaster) {

    // init
    $scope.entity = { Name: "Fees Heads" };
    ...
    $scope.GetAll = function () {
        ...
        feesHeadsService.query(function (result) {
            $scope.feesHeads = result;
            $scope.items = $scope.feesHeads;
            console.log($scope.items);
            $scope.query = "";            
            $scope.search();
            $scope.isLoading = false;
    ...
    // inline Add /Edit
    $scope.GetAll();
    $scope.feesHeads = GetNewModel();
    $scope.Add = function (item) {

        console.log(item);
        feesHeadsService.save(item, function (result) {
            $scope.feesHeads = GetNewModel();
            $scope.isNew = false;
            $scope.GetAll();
    ...
    $scope.Update = function (item) {
        feesHeadsService.update({ id: item.Id }, item, function (result) {
...
//'use strict';
///* Controllers */
//app.controller('feesHeads', ['$scope', '$state', '$http', 'feesHeadsService', 'modalService', '$filter', 'toaster',
//    function ($scope, $state, $http, feesHeadsService, modalService, $filter, toaster) {

//        $scope.init = function () {
//            feesHeadsService.getFeesHead().then(function (result) {
//                $scope.heads = result;
//            })
...
//        }
//        $scope.save = function (feesHead) {
//            debugger;
//            console.log(feesHead);
//            feesHeadsService.save(feesHead).then(function (result) {
//                $scope.heads.push(result);
//            });
//        }
