...
namespace pnsms.Service
{
    ...
    public class InstituteService : Service<Institute>, IInstituteService
    {
        ...
        private readonly IRepositoryAsync<Institute> _repository;
        ...
        public InstituteService(IRepositoryAsync<Institute> repository, IPackageService iPackageService, IImageService imageService,
        ...
        {
            var institute = _repository.Query(x => x.Id == id).Select().FirstOrDefault();
        ...
        {
            return _repository.Query().Include(x => x.AcademicBranches).Select().FirstOrDefault(x => x.Id == id && x.IsActive == true);
        }
        public Institute GetActiveInstituteByIdForApi(int id)
        {
            return _repository.Query(x => x.Id == id && x.IsActive == true).Select().FirstOrDefault();
        }
        public Institute Get(string url)
        {
            return _repository.Query().Select().FirstOrDefault(x => x.Url == url);
        }
        public List<KeyValuePair<int, string>> GetKVP()
        {
            var data = _repository.Query(c => c.GlobalDistrictId==16).Select().ToList();
