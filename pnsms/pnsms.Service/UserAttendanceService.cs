...
namespace pnsms.Service
{
    ...
    {
        private readonly IRepositoryAsync<UserAttendance> _repository;
        private readonly IUserAttendanceDetailService _IUserAttendanceDetailService;
        public UserAttendanceService(IRepositoryAsync<UserAttendance> repository
        ...
        public IEnumerable<UserAttendance> GetAllTeacherAttendance(VmSearchAttendance vmSearchAttendance)
        {
            ...
            return _repository.Query(p => p.AttendanceDate >= start.Date
        ...
        public UserAttendance GetTeacherAttendance(VmUserAttendance vmUserAttendance)
        {
            ...
            return _repository.Query(p => EntityFunctions.TruncateTime(p.AttendanceDate) == EntityFunctions.TruncateTime(vmUserAttendance.UserAttendance.AttendanceDate.Date)
                                        && p.UserInfoTypeId == RefTypeId
        ...
        {
            var objUserAttendance = _repository.Query(x => x.Id == attendanceId)
        ...
        //{
        //    _repository.Query(x=>x.AttendanceDate==).Select().Max(x=>x.AttendanceDate)
        //}
    }
